...
def prepare_intersection_data(cancerType):
    ppi, ppi_nodes = load_network('./TGBWDriver-master/data/CPDB.txt')
    rna_file = f'./TGBWDriver-master/{cancerType}/orig_data/exp.txt'
    snp_file = f'./TGBWDriver-master/{cancerType}/orig_data/mut.txt'
    ...
    ppi.to_csv(path_or_buf=f'./TGBWDriver-master/{cancerType}/orig_data/CPDB.txt', sep='\t', header=False, index=False)
...
def create_mutation_and_driver_matrices(cancerType):
    ...
    gold_drivers_file = f'./TGBWDriver-master/{cancerType}/Cancer_List/driver_list.txt'
    ...
    ppi_file = f'./TGBWDriver-master/{cancerType}/orig_data/CPDB.txt'
    ...
    output_dir = f'./TGBWDriver-master/{cancerType}/Cancer_List/'
    P.to_csv(path_or_buf=output_dir + 'mut_driver.txt', sep='\t', header=True, index=True)
    exp.to_csv(path_or_buf=output_dir + 'exp.txt', sep='\t', header=True, index=True)
    snp.to_csv(path_or_buf=output_dir + 'mut.txt', sep='\t', header=True, index=True)
