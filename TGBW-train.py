...
cancerType = "BRCA"
DATA_ROOT = r"./TGBWDriver-master"
OUT_DIR = f"{DATA_ROOT}/{cancerType}/Cancer_List/result/TGBW"
KFOLDS = 5
FAST_MODE = True
EPOCHS = 500
PATIENCE = 20
...
# ========= 读数据 =========
exp = pd.read_table(f"{DATA_ROOT}/{cancerType}/Cancer_List/exp.txt", header=0, index_col=0, sep="\t")
mut = pd.read_table(f"{DATA_ROOT}/{cancerType}/Cancer_List/mut.txt", header=0, index_col=0, sep="\t")
adj_df = pd.read_table(f"{DATA_ROOT}/{cancerType}/Cancer_List/mut_driver.txt", header=0, index_col=0, sep="\t")
...
# 固定邻居数目
NUM_NEIGHBORS = 30 # 每个节点固定选择30个邻居
...
for fold_id, test_cols in enumerate(folds, 1):
    ...
    pred_full = train_one_fold(model)
