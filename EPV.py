...
# -------- 读取数据 --------
snp = pd.read_table(
    filepath_or_buffer=f'./TGBWDriver-master/{cancerType}/Cancer_List/mut.txt',
    header=0, index_col=0, sep='\t'
)
final_result = pd.read_table(
    filepath_or_buffer=f'./TGBWDriver-master/{cancerType}/Cancer_List/result/TGBW/final_result_with_attention.txt',
    header=0, index_col=0, sep='\t'
)
...
def calculate_weighted_condorcet(patient_genes, candidate_names):
    ...
    for ids in rank_lists:
        if not ids:
            continue
        n = len(ids)
        weights = np.exp(-0.9* np.arange(n))
...
top_500_genes.to_csv(
    f'./{cancerType}/Cancer_List/result/TGBW/score.txt',
    sep='\t'
)

print(f"\n前500个驱动基因已保存到文件!")
print(f"文件路径: ./{cancerType}/Cancer_List/result/TGBW/score.txt")
